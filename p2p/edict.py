...
# Build our SD model
unet = UNet2DConditionModel.from_pretrained(model_path_diffusion, subfolder="unet", revision="fp16",
...
def image_to_latent(im, width, height):
    if isinstance(im, torch.Tensor):
        ...
        im = im.to(device)
        # Encode image
        init_latent = vae.encode(im).latent_dist.sample() * 0.18215
        return init_latent
...
def coupled_stablediffusion(prompt="",
    ...
    if init_image is not None:
        ...
        if isinstance(init_image, list):
            if isinstance(init_image[0], torch.Tensor):
                ...
                init_latent = [image_to_latent(im, width, height) for im in init_image]
        else:
            init_latent = image_to_latent(init_image, width, height)
    ...
    if reverse:
        ...
        noise = torch.randn(init_latent.shape, device=device, dtype=torch.float16)
