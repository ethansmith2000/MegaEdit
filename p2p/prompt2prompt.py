...
class LocalBlend:
    ...
    def __init__(self, prompts: List[str], words: [List[List[str]]], tokenizer, NUM_DDIM_STEPS, substruct_words=None, start_blend=0.2,
                 th=(.3, .3), MAX_NUM_WORDS = 77, device = None, dtype=None):
        ...
        if substruct_words is not None:
            ...
            self.substruct_layers = substruct_layers.to(device).to(dtype)
        else:
...
class AttentionControlEdit(AttentionStore, abc.ABC):
    ...
    def __init__(self, prompts, num_steps: int, tokenizer,
        ...
        self.cross_replace_alpha = ptp_utils.get_time_words_attention_alpha(prompts, num_steps, cross_replace_steps, tokenizer).to(device).to(dtype)
...
class AttentionRefine(AttentionControlEdit):
    ...
    def __init__(self, prompts, num_steps: int, tokenizer, cross_replace_steps: float, self_replace_steps: float,
                 local_blend: Optional[LocalBlend] = None, device=None, dtype=None):
        super(AttentionRefine, self).__init__(prompts, num_steps, tokenizer, cross_replace_steps, self_replace_steps, local_blend, device=device, dtype=dtype)
...
class AttentionReweight(AttentionControlEdit):
    ...
    def __init__(self, prompts, num_steps: int, tokenizer, cross_replace_steps: float, self_replace_steps: float, equalizer,
                 local_blend: Optional[LocalBlend] = None, controller: Optional[AttentionControlEdit] = None, device=None, dtype=None):
...
def make_controller(prompts, tokenizer, NUM_DDIM_STEPS, is_replace_controller: bool, cross_replace_steps: Dict[str, float],
                    self_replace_steps: float, blend_words=None, equalizer_params=None, substruct_words=None, start_blend=0.2, th=(.3, .3), device=None, dtype=None) -> AttentionControlEdit:
    if blend_words is None:
        lb = None
    else:
        lb = LocalBlend(prompts, blend_words, tokenizer, NUM_DDIM_STEPS, substruct_words=substruct_words, start_blend=start_blend,th=th, device=device,dtype=dtype)
    ...
    if equalizer_params is not None:
        eq = get_equalizer(prompts[1], tokenizer, equalizer_params["words"], equalizer_params["values"], device=device, dtype=dtype)
...
class AttentionJustReweight:
    ...
    def __call__(self, attn, is_cross: bool, place_in_unet: str):
        if self.cur_att_layer >= self.num_uncond_att_layers:
            attn = attn[None, :, :, :] * self.equalizer[:, None, None, :]
            # attn_replace = attn_replace / attn_replace.sum(-1, keepdims=True)
        self.cur_att_layer += 1
        if self.cur_att_layer == self.num_att_layers + self.num_uncond_att_layers:
            self.cur_att_layer = 0
    ...
    def __init__(self, batch_size, num_steps: int, cross_replace_steps: float, equalizer, device=None, dtype=None):
        self.equalizer = equalizer.repeat(batch_size*2, 1).to(device).to(dtype)
